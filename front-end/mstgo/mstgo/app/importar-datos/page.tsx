  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.name.toUpperCase() !== 'DATA.TXT') {
        ...
        setStatusMessage('Error: El archivo debe llamarse DATA.TXT')
        return
      }
      ...
      try {
        ...
        if (error instanceof TypeError && error.message === 'Failed to fetch') {
          setStatusMessage('Error: No se pudo conectar con el servidor. Por favor, asegúrese de que el servidor esté en ejecución y sea accesible.');
        } else {
          setStatusMessage(`Error al subir el archivo: ${error instanceof Error ? error.message : 'Error desconocido'}`);
        }
      } finally {
        setIsLoading(false)
      }
      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="flex flex-1 pt-16">
          ...
          <main className="flex-1 overflow-y-auto p-6 bg-gray-100">
            ...
            {uploadStatus !== 'idle' && (
                <Alert variant={uploadStatus === 'error' ? "destructive" : "default"} className="mb-4">
                  <AlertCircle className="h-4 w-4" />
                  <AlertTitle>{uploadStatus === 'success' ? 'Éxito' : 'Estado de la subida'}</AlertTitle>
